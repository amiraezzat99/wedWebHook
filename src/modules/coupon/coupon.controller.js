...
export const createCoupon = async (req, res, next) => {
    ...
    if (!users.length) {
        return next(new Error('in-valid Ids'))
    }
    ...
    if (
        ...
        return next(new Error('toDate must be greater than from date', { cause: 400 }))
    }
...
export const updateCoupon = async (req, res, next) => {
    ...
    const { matched, expired, exceed } = validateCoupon(coupon, req.user._id, next)
    if (!matched) {
        return next(new Error('this coupon does not assgined to you', { cause: 400 }))
    }
    if (exceed) {
        return next(new Error('you exceed the maxUsage of this coupon', { cause: 400 }))
    }
    if (expired) {
...
//============== validate coupon ===========
export const validateCoupon = (coupon, loggedInUserId) => {
     (coupon);
     (loggedInUserId);
    // expirationDate
    let expired = false
    if (moment(coupon.toDate).isBefore(moment()) || coupon.couponStatus == 'expired') {
        // return next(new Error('coupon is expired', { cause: 400 }))
        expired = false
    }
    // Assgined User   => [ users] , user
    let matched = false
    let exceed = false
    for (const assginedUser of coupon.usagePerUser) {
        if (assginedUser.userId.toString() == loggedInUserId) {
            matched = true
            if (assginedUser.usageCount >= assginedUser.maxUsage) {
                exceed = true
            }
        }
    }
    // maxUsage Vs usageCount
     ({ matched, exceed });
    // if (!matched) {
    //     // return next(new Error('this coupon does not assgined to you', { cause: 400 }))
    // }
